...
bool monstersAlive(const std::array<Monster *,4> &monsters, int n)
{
  for (int i = 0 ; i < n ; i++)
...
int main() {
  ...
  std::array<Monster *,4> m{new Monster{"Normal Monster"}, 
	  new DefenseMonster{"Defense Monster"}, 
	  new StrongMonster{"Strong Monster"}, 
	  new RecStrongMonster{"Rec Strong Monster"}};

  cout << "Você está encurralado em um beco." << endl;

  cout << "Existem 4 monstros prontos para atacar, e você não pode correr." << endl << endl;

  while (monstersAlive(m,4) && life>0)
  {
    cout << *m[0] << *m[1] << *m[2] << *m[3] << endl;

    cout << "Você" << endl << "Life: " << life << endl << endl;

    cout << "Que monstro quer atacar (0 - 3)? ";
    int op;
    cin >> op;
